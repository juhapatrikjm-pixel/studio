    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 pb-20">
      ...
      <Tabs value={activeTab} onValueChange={(val: any) => { setActiveTab(val); setCurrentFolderId(null); }} className="w-full">
        <TabsList className="grid w-full grid-cols-3 mb-8 bg-black/40 border border-white/5 p-1 h-12">
          ...
          <TabsTrigger value="uploads" className="text-[10px] font-black uppercase tracking-widest">LADATUT RESEPTIT</TabsTrigger>
        </TabsList>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                <div 
                  ...
                  className={cn(
                    "flex items-center gap-3 p-2.5 rounded-xl cursor-pointer text-[10px] font-black uppercase tracking-tight transition-all", 
                    !currentFolderId ? "bg-primary/20 text-accent border border-accent/20" : "text-muted-foreground hover:bg-white/5"
                  )}
                >
                ...
                {filteredFolders.map(f => (
                  <div 
                    ...
                    className={cn(
                      "group flex justify-between items-center p-2.5 rounded-xl cursor-pointer text-[10px] font-black uppercase tracking-tight border transition-all", 
                      currentFolderId === f.id ? "bg-primary/20 text-accent border-accent/20" : "text-muted-foreground border-transparent hover:bg-white/5"
                    )}
                  >
          ...
          <Card className="lg:col-span-3 industrial-card relative overflow-hidden min-h-[500px]">
            <div className="absolute top-0 left-0 w-1 h-full copper-gradient opacity-20" />
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-background border-white/10">
                          ...
                          <DropdownMenuItem 
                            ...
                            onClick={() => deleteItem(item.id, item.colName)}
                          >
                ...
                {currentItems.length === 0 && filteredFolders.length === 0 && (
                  <div className="col-span-full py-32 text-center border-2 border-dashed border-white/5 rounded-3xl opacity-20">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em]">Tämä kansio on tyhjä.</p>
                  </div>
                )}
              </div>
            </CardContent>
